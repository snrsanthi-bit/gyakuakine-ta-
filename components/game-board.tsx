...
export function GameBoard() {
  ...
  if (!game) return <section className="rounded-3xl bg-white p-8 text-center shadow-xl shadow-indigo-100"><p className="text-sm font-bold tracking-widest text-indigo-600">REVERSE AKINATOR</p><h1 className="mt-3 text-3xl font-black text-slate-900">AIのお題を当てよう</h1><p className="mx-auto mt-4 max-w-md text-slate-600">AIは「はい」「いいえ」「どちらとも言えない」だけで答えます。</p><div className="mx-auto mt-6 max-w-md rounded-xl bg-amber-50 p-4 text-left text-sm text-amber-800"><p className="font-bold">⚠️ 試作版につき、ご利用回数に制限があります。</p><p className="mt-2"> AI利用にはコストがかかるため、ご利用回数に制限があります。エラーが表示された場合は、時間を空けて再度お試しください。</p></div>{error && <p role="alert" className="mt-4 text-sm font-bold text-rose-600">{error}</p>}<button data-testid="start-game" onClick={() => act("/api/game/start", {})} disabled={sending} className="mt-7 rounded-xl bg-indigo-600 px-6 py-3 font-bold text-white hover:bg-indigo-700 disabled:bg-slate-300">ゲームを始める</button></section>;
